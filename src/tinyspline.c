...
void ts_internal_bspline_find_u(tsBSpline spline, tsReal u, size_t *k,
	...
	tsBSpline *_result_, size_t* k)
{
	...
	TRY(buf, err)
		...
		ts_internal_bspline_insert_knot(spline, net, n, _result_, buf);
		*k = net.pImpl->k+n;
	CATCH
		if (spline.pImpl != _result_->pImpl)
			ts_bspline_default(_result_);
...
void ts_internal_bspline_split(tsBSpline spline, tsReal u, tsBSpline *_split_,
	size_t* k, jmp_buf buf)
{
	tsDeBoorNet net;
	tsError e;
	jmp_buf b;
...
void ts_internal_bspline_to_beziers(tsBSpline spline, tsBSpline *_beziers_,
	jmp_buf buf)
{
	tsError e;
	jmp_buf b;
