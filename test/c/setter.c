...
void setter_knots_interval_0_1(CuTest *tc)
{
...
void setter_knots_interval_1_2(CuTest *tc)
{
	tsBSpline spline = ts_bspline_init();
	tsReal *knots = NULL;
	tsError err;
	...
	CuAssertDblEquals(tc, 1.f,   knots[3],  EPSILON);
	...
	CuAssertDblEquals(tc, 2.f,   knots[8],  EPSILON);
	CuAssertDblEquals(tc, 2.f,   knots[9],  EPSILON);
	CuAssertDblEquals(tc, 2.f,   knots[10], EPSILON);

	ts_bspline_free(&spline);
	free(knots);
}

void setter_knots_interval_negative_9_5(CuTest *tc)
{
	tsBSpline spline = ts_bspline_init();
	tsReal *knots = NULL;
	tsError err;
	...
	err = ts_bspline_knots(&spline, &knots, NULL);
	CuAssertIntEquals(tc, TS_SUCCESS, err);
	CuAssertDblEquals(tc, -9.f, knots[0],  EPSILON);
	CuAssertDblEquals(tc, -9.f, knots[1],  EPSILON);
	CuAssertDblEquals(tc, -9.f, knots[2],  EPSILON);
	CuAssertDblEquals(tc, -9.f, knots[3],  EPSILON);
	CuAssertDblEquals(tc, -8.f, knots[4],  EPSILON);
	CuAssertDblEquals(tc, -7.f, knots[5],  EPSILON);
	CuAssertDblEquals(tc, -6.f, knots[6],  EPSILON);
	CuAssertDblEquals(tc, -5.f, knots[7],  EPSILON);
	CuAssertDblEquals(tc, -5.f, knots[8],  EPSILON);
	CuAssertDblEquals(tc, -5.f, knots[9],  EPSILON);
	CuAssertDblEquals(tc, -5.f, knots[10], EPSILON);
...
CuSuite* get_setter_suite()
{
	...
	SUITE_ADD_TEST(suite, setter_knots_interval_0_1);
	SUITE_ADD_TEST(suite, setter_knots_interval_1_2);
	SUITE_ADD_TEST(suite, setter_knots_interval_negative_9_5);
	return suite;
}