...
void new_test_bspline_deg_greater_nctrlp(CuTest* tc)
{
    ...
    ctests_init_non_default_bspline(&bspline);
    err = ts_bspline_new(3, 1, 2, TS_NONE, &bspline);
    CuAssertIntEquals(tc, TS_DEG_GE_NCTRLP, err);
    ctests_assert_default_bspline(tc, &bspline);

    ctests_init_non_default_bspline(&bspline);
    err = ts_bspline_new(6, 2, 4, TS_NONE, &bspline);
    CuAssertIntEquals(tc, TS_DEG_GE_NCTRLP, err);
    ctests_assert_default_bspline(tc, &bspline);

    ctests_init_non_default_bspline(&bspline);
    err = ts_bspline_new(10, 3, 3, TS_NONE, &bspline);
    CuAssertIntEquals(tc, TS_DEG_GE_NCTRLP, err);
    ctests_assert_default_bspline(tc, &bspline);

    ctests_init_non_default_bspline(&bspline);
    err = ts_bspline_new(100, 4, 90, TS_NONE, &bspline);
    CuAssertIntEquals(tc, TS_DEG_GE_NCTRLP, err);
    ctests_assert_default_bspline(tc, &bspline);
}
...
void new_test_bspline_beziers_setup_failed(CuTest* tc)
{
    ...
    ctests_init_non_default_bspline(&bspline);
    err = ts_bspline_new(3, 3, 7, TS_BEZIERS, &bspline);
    CuAssertIntEquals(tc, TS_NUM_KNOTS, err);
    ctests_assert_default_bspline(tc, &bspline);

    ctests_init_non_default_bspline(&bspline);
    err = ts_bspline_new(3, 2, 11, TS_BEZIERS, &bspline);
    CuAssertIntEquals(tc, TS_NUM_KNOTS, err);
    ctests_assert_default_bspline(tc, &bspline);

    ctests_init_non_default_bspline(&bspline);
    err = ts_bspline_new(5, 1, 15, TS_BEZIERS, &bspline);
    CuAssertIntEquals(tc, TS_NUM_KNOTS, err);
    ctests_assert_default_bspline(tc, &bspline);
}
